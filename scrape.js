...
const scrape = async () => {
  ...
  while(currentPage <= maxPages){
    ...
    const sidebarCategories = await page.evaluate(() => {
      const sidebarElement = document.querySelector(".side_categories ul");
      return Array.from(sidebarElement.querySelectorAll("li a")).map((category) => {
        return {
  ...
  fs.writeFileSync('books.json', JSON.stringify({allBooks, sidebarCategories}, null, 2));
